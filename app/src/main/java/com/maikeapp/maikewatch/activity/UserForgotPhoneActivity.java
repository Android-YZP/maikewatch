            ...
            private void checkPhoneFromNet(final String phone) {
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            //检查是否存在该手机号码
                            String _result = mUserBusiness.getMobileNumberIsVaild(phone);

                            JSONObject object = new JSONObject(_result);
                            Boolean success = JsonUtils.getBoolean(object, "Success");
                            Log.e("_result", _result + "YZP" + success);
                            if (success) {//success为false时才可以修改
                                CommonUtil.sendErrorMessage("手机号码未注册,清先注册", handler);
                                return;
                            }
                            String _resultCode = sendSMSToPhone();//发送验证码短信

//                            String result = mUserBusiness.getUserForgotPasswordOne(phone);
                            JSONObject jsonObj = new JSONObject(_resultCode);
